...
echo "$($PSQL "select MAX(DISTINCT(winner_goals)) from games")"
...
echo "$($PSQL "select year, name from games as g inner join teams as t on g.winner_id = t.team_id where round = 'Final' order by year, name")"
